    ...
    public class SnowflakeService
    {
        ...
        public async Task<DataTable> QueryAsync(string sql)
        {
            ...
            var csb = new SnowflakeDbConnectionStringBuilder
            {
                ...
                ["HOST"] = $"{cfg["Account"]}.snowflakecomputing.com", // ← THIS FIXES the RemoteCertificateNameMismatch
                ["USER"] = cfg["User"],
                ["WAREHOUSE"] = cfg["Warehouse"],
                ["ROLE"] = cfg["Role"],
                ["AUTHENTICATOR"] = "snowflake_jwt",        // lowercase also works, both are accepted
                ["PRIVATE_KEY"] = keyText,                // full PEM string (with -----BEGIN...-----)
                ["PRIVATE_KEY_PWD"] = passphrase           // your passphrase
            };
